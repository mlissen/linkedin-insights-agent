...
import type { AuthContext, RunConfigInput } from '../types.js';
...
export async function listRuns(userId: string) {
  ...
  const { data, error } = await supabase
    .from('runs')
    .select(
      'id, status, run_nickname, created_at, completed_at, failure_reason, token_estimate, cost_estimate',
    )
    .eq('user_id', userId)
...
export async function getRun(userId: string, runId: string) {
  ...
  const { data, error } = await supabase
    .from('runs')
    .select(
      'id, status, config, run_nickname, created_at, completed_at, failure_reason, token_estimate, cost_estimate, needs_login_url',
    )
    .eq('user_id', userId)
    .eq('id', runId)
    .single();
