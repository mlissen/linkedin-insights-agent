...
describe('processRunJob', () => {
    beforeEach(() => {
        ...
        supabaseMod.getSupabase.mockReturnValue(supabaseStub);
        Queue.mockReturnValue({ add: vi.fn() });
    });
    it('requests login when no active session', async () => {
        loginMod.ensureLoginSession.mockResolvedValue(undefined);
        loginMod.decryptCookies.mockReturnValue([]);
