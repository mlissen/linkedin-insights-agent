...
import {
  ...
  type InsightAnalysisLike,
  type FormatterLike,
} from './knowledge-bundler.js';
...
type AnalyzerLike = {
  analyzeInsights(posts: unknown[], config: Record<string, unknown>): Promise<InsightAnalysisLike>;
};

async function loadInsightAnalyzerCtor(): Promise<new () => AnalyzerLike> {
  const modulePath = '../../../src/' + 'analyzer.js';
  const mod = (await import(modulePath)) as any;
  return mod.InsightAnalyzer as new () => AnalyzerLike;
}

async function loadFormatterInstance(): Promise<FormatterLike> {
  const modulePath = '../../../src/' + 'three-file-formatter.js';
  const mod = (await import(modulePath)) as any;
  return new mod.ThreeFileFormatter() as FormatterLike;
}
...
export async function processRunJob(data: RunJobData, job: Job<RunJobData>) {
  ...
  try {
    ...
    const scrapeResult = await scraper.scrapeProfiles(
      run.config.profileUrls,
      run.config.postLimit,
      run.config.topics,
    );
    ...
    const perExpertAnalysis: Record<string, InsightAnalysisLike> = {};
